Shader "CommandBufferTest/Grab/Distort"
{
	Properties
	{
	...
	SubShader
	{
		...
		Pass
		{
			...
			sampler2D _GrabDisortTexture;
			float4 _GrabDisortTexture_TexelSize;
			...
			fixed4 frag (v2f i ) : SV_Target
			{
				...
				float4 screenColor50 = tex2D(_GrabDisortTexture, ( ( (ase_grabScreenPosNorm).xy / ase_grabScreenPosNorm.a ) + uv_refraction ) );

				//i.uvgrab.xy = 0.2 * i.uvgrab.z + i.uvgrab.xy;
				//half4 screenColor50 = tex2Dproj(_GrabDisortTexture, UNITY_PROJ_COORD(i.uvgrab + uv_refraction));
